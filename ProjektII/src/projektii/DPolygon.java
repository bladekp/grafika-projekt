...
public class DPolygon {
	...
	{		
		...
		for(int i=0; i<x.length; i++)
		{
			...
			newX[i] = (ProjektII.ScreenSize.getWidth()/2 - Calculator.CalcFocusPos[0]) + CalcPos[0] * Screen.zoom;
			newY[i] = (ProjektII.ScreenSize.getHeight()/2 - Calculator.CalcFocusPos[1]) + CalcPos[1] * Screen.zoom;			
			if(Calculator.t < 0)
				draw = false;
		}
	...
	void calcLighting()
	{
		...
		double angle = Math.acos(((lightingPlane.NV.x * Screen.LightDir[0]) + (lightingPlane.NV.y * Screen.LightDir[1]) + (lightingPlane.NV.z * Screen.LightDir[2]))
		...
		//DrawablePolygon.lighting = 0.2 + 1 - Math.sqrt(Math.toDegrees(angle)/180);
                DrawablePolygon.lighting = angle /2;
                
                
		if(DrawablePolygon.lighting > 1.57)
			DrawablePolygon.lighting = 1.57;
